...
docker-test-run:
	docker run --name ${BINARY_NAME}-testing1 ${BINARY_NAME}-testing:latest -version
	docker run --name ${BINARY_NAME}-testing2 -e GITHUB_ACTIONS=true ${BINARY_NAME}-testing:latest -compare -version1 v1.2.0 -version2 v1.2.5
	docker run --name ${BINARY_NAME}-testing3 -e GITHUB_ACTIONS=true ${BINARY_NAME}-testing:latest -core -block minor -version1 v1.11.0

docker-test-remove:
	docker stop ${BINARY_NAME}-testing1 ${BINARY_NAME}-testing2 ${BINARY_NAME}-testing3
	docker rm ${BINARY_NAME}-testing1 ${BINARY_NAME}-testing2 ${BINARY_NAME}-testing3

docker-test-all: docker-test-build docker-test-run docker-test-remove